...
def downsample(in_planes, out_planes):
    net = nn.Sequential(nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0), nn.MaxPool2d(kernel_size=2, stride=2))
    return net;


def upsample(in_planes, out_planes):
    net = nn.Sequential(nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0), nn.UpsamplingBilinear2d(scale_factor=2))
    return net;
